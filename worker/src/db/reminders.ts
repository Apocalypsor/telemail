...
export const listPendingRemindersForEmail = async (
  d1: D1Database,
  telegramUserId: string,
  accountId: number,
...
export const countPendingRemindersForEmail = async (
  d1: D1Database,
  accountId: number,
  emailMessageId: string,
): Promise<number> => {
...
export const updateReminderThingsTaskId = async (
  d1: D1Database,
  id: number,
  thingsTaskId: string,
): Promise<void> => {
...
export const deletePendingReminder = async (
  d1: D1Database,
  telegramUserId: string,
  id: number,
): Promise<boolean> => {
...
/** 统计某用户待发送提醒数量（用于上限保护） */
export const countPendingReminders = async (
  d1: D1Database,
  telegramUserId: string,
): Promise<number> => {
  const db = getDb(d1);
