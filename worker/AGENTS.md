...
- **IMAP bridge client**: talk to middleware through Eden treaty (`bridgeClient` / `bridgeCall` in `providers/imap/`), typed against `import type { App } from "@middleware/index"`. Treaty config sets `throwHttpError: true` so non-2xx auto-throws `EdenFetchError`. For raw streaming routes that Eden cannot model cleanly, use `bridgeFetch` + `bridgeRequestUrl`; never import container runtime files just to get URL constants.
