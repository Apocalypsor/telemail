...
- **`api/`** — HTTP layer (Elysia). Layout (modules + plugins) in root [AGENTS.md](../AGENTS.md) "Elysia layout". Plugins in use: `cf` (env + waitUntil), `auth-{session,miniapp,any}`, `secrets`. `api/index.ts` composes everything with `CloudflareAdapter` + `.compile()` and exports `type App` for Eden.
