...
CI/CD（`.github/workflows/ci.yml`）：push to `main` → 按 path filter 自动部署改动的 workspace（worker `wrangler deploy`，其中包含 IMAP Container 镜像构建 / page `wrangler pages deploy --branch=main`）。PR 自动出 preview deploys（worker version preview URL + Pages preview branch），URL 会 sticky comment 到 PR 上。
...
- **IMAP**：Cloudflare Container 中的 IMAP Bridge 检测新邮件 → Worker 内部 `/api/imap/push` handler → `ImapProvider.enqueue()` 验证后入队。
