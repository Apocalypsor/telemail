...
- **前端** (`web/`): [Vite](https://vite.dev) + React 19 + TypeScript + [TanStack Router](https://tanstack.com/router) + [TanStack Query](https://tanstack.com/query) + [ky](https://github.com/sindresorhus/ky) + [zod](https://zod.dev)，pnpm workspace
...
- `example.com/api/*`、`/webhook*`、`/oauth/*`、`/login*`、`/preview*`、`/junk-check*`、`/mail/*` → Worker（wrangler 部署）
- `example.com/telegram-app/*`、`/` 等其它 → Pages（Git 接入，自动部署 `web/dist`）
...
# 前端（Pages，web/ 子包）
pnpm dev:web    # Vite dev server (127.0.0.1:5173)，/api/* 自动代理到本地 wrangler
pnpm build:web  # 构建到 web/dist，用于 Pages 部署
...
2. 另一个终端跑 `pnpm dev:web`（Vite 起在 :5173）
...
2. Build command: `corepack enable && pnpm install --filter telemail-web && pnpm --filter telemail-web build`
3. Build output directory: `web/dist`
4. 绑定自定义域名
5. 在 Workers & Pages → Workers → 你的 Worker → Settings → Triggers → Routes，加上：`example.com/api/*`、`example.com/oauth/*`、`example.com/login*`、`example.com/preview*`、`example.com/mail/*`、`example.com/junk-check*`
