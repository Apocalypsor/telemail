...
- **`middleware/`** —— IMAP bridge（Bun + Elysia + ImapFlow）。生产默认由 `worker/` 的 Cloudflare Container binding 运行；也保留 Docker Compose 自托管 fallback。`bun build --compile` 出单文件 binary，distroless 镜像。详见 [`DEPLOYMENT.md` §6.4](./DEPLOYMENT.md)。
