...
export async function getAllFailedEmails(
  ...
  const { results } = await db
    .prepare("SELECT * FROM failed_emails ORDER BY created_at DESC")
    .all<FailedEmail>();
  return results;
}
...
export async function getFailedEmail(
  ...
  return db
    .prepare("SELECT * FROM failed_emails WHERE id = ?")
    .bind(id)
    .first<FailedEmail>();
}
...
export async function deleteFailedEmailsByAccountId(
  ...
  await db
    .prepare("DELETE FROM failed_emails WHERE account_id = ?")
    .bind(accountId)
    .run();
}
...
export async function countFailedEmails(db: D1Database): Promise<number> {
  const row = await db
    .prepare("SELECT COUNT(*) as cnt FROM failed_emails")
    .first<{ cnt: number }>();
  return row?.cnt ?? 0;
}
