...
export async function cleanupAndDeleteAccount(env: Env, account: Account): Promise<void> {
	// 停止邮件推送
	if (account.type === AccountType.Imap) {
		...
		if (env.IMAP_BRIDGE_URL && env.IMAP_BRIDGE_SECRET) {
			await syncAccounts(env).catch((err) => {
				reportErrorToObservability(env, 'imap.sync_after_delete_failed', err, { accountId: account.id });
			});
		}
		...
		await Promise.all([deleteAccount(env.DB, account.id), deleteHistoryId(env, account.id)]);
	}
...
/** 删除用户及其绑定的所有邮箱账号 */
export async function deleteUserWithAccounts(env: Env, telegramId: string): Promise<void> {
