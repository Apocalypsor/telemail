		...
		<Layout title="Gmail OAuth 授权">
			<Card class="max-w-3xl">
				...
				<ol class="mt-3 ml-5 space-y-2 list-decimal text-sm text-slate-400 leading-relaxed">
					...
					<li>
						保存后调用 <code class="px-1.5 py-0.5 bg-slate-900 rounded text-blue-300 text-xs break-all">{watchUrl}</code> 续订 watch。
					</li>
				</ol>
				<a
...
export function OAuthCallbackPage({
	...
	const statusText = refreshToken
		? `已为 ${accountEmail} 保存 refresh_token 到数据库，后续会自动使用。`
		...
		<Layout title={title}>
			<Card class="max-w-3xl">
				...
				<ol class="ml-5 list-decimal text-sm text-slate-400">
					<li>续订 Gmail watch（会为所有已授权账号续订）：</li>
				</ol>
