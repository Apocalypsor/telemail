...
(function () {
  ...
  if (!sp) {
    location.replace(${JSON.stringify(ROUTE_MINI_APP_REMINDERS)});
    return;
  }

  // 解析 [<feature>_]<chatId>_<tgMsgId>
  var m = sp.match(/^(?:([a-z])_)?(-?\\d+)_(\\d+)$/);
  if (!m) { fail('无效的入口参数'); return; }
  var feature = m[1] || 'r';  // 默认 r（兼容旧无前缀按钮）
  var chatId = m[2];
  var tgMsgId = m[3];

  fetch(${JSON.stringify(ROUTE_REMINDERS_API_RESOLVE_CONTEXT)} + '?start=' + encodeURIComponent(chatId + '_' + tgMsgId), {
    ...
    .then(function (res) {
      ...
      var d = res.data;
      if (feature === 'm') {
        location.replace(${JSON.stringify(ROUTE_MINI_APP_MAIL.replace(":id", ""))} + encodeURIComponent(d.emailMessageId)
          + '?accountId=' + d.accountId + '&t=' + encodeURIComponent(d.token));
      } else {
        location.replace(${JSON.stringify(ROUTE_MINI_APP_REMINDERS)} + '?accountId=' + d.accountId
...
export function MiniAppRouterPage() {
  return (
    <MiniAppShell title="Telemail">
      <p style="padding:20px;color:var(--hint);">加载中…</p>
