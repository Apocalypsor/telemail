...
export const notifyHaptic = (kind: "success" | "warning" | "error"): void => {
...
export const alertPopup = async (message: string): Promise<void> => {
  if (showPopup.isAvailable()) {
    try {
      await showPopup({
        message,
 ...
 *  其他环境用新窗口。 */
export const openTgLink = (url: string): void => {
...
export const closeMiniAppSafe = (): void => {
