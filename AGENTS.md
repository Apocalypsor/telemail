...
- **Web 页面** (`page/src/routes/`，不在 `telegram-app/` 子目录下): 浏览器里直接访问的页面，`WebLayout` 套 sticky emerald wordmark header，HeroUI Card / Button 做现代化外观。
  ...
  - **Types sharing**: `page/tsconfig.json` + `page/vite.config.ts` alias `@worker/*` → `../worker/*`; page uses `import type` only. `page/src/api/routes.ts` re-exports Worker's pure-string route constants so both sides stay in sync.
